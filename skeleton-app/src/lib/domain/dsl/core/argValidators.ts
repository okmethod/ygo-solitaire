...
export const ArgValidators = {
   ...
   */
  positiveInt(args: DSLArgs, key: string): number {
   ...
   */
  nonNegativeInt(args: DSLArgs, key: string): number {
    const value = args[key];
   ...
   * 正の整数を取得（オプション）
   */
  optionalPositiveInt(args: DSLArgs, key: string): number | undefined {
    const value = args[key];
   ...
   */
  string(args: DSLArgs, key: string): string {
    const value = args[key];
   ...
   * 文字列を取得（オプション）
   */
  optionalString(args: DSLArgs, key: string): string | undefined {
    const value = args[key];
    if (value === undefined) return undefined;
    if (typeof value !== "string") {
   ...
   */
  nonEmptyString(args: DSLArgs, key: string): string {
    const value = args[key];
   ...
   */
  boolean(args: DSLArgs, key: string): boolean {
    const value = args[key];
   ...
   */
  optionalBoolean(args: DSLArgs, key: string, defaultValue: boolean): boolean {
    const value = args[key];
   ...
   */
  oneOf<T extends readonly string[]>(args: DSLArgs, key: string, validValues: T): T[number] {
    const value = args[key];
   ...
   */
  player(args: DSLArgs, key: string): Player {
    const value = args[key];
   ...
   */
  optionalPlayer(args: DSLArgs, key: string, defaultValue: Player): Player {
    const value = args[key];
    if (value === undefined) return defaultValue;
    if (value !== "player" && value !== "opponent") {
   ...
   * カードタイプを取得（オプション）
   */
  optionalCardType(args: DSLArgs, key: string): CardType | undefined {
    const value = args[key];
    if (value === undefined) return undefined;
   ...
   * 魔法カードサブタイプを取得（オプション）
   */
  optionalSpellSubType(args: DSLArgs, key: string): SpellSubType | undefined {
    const value = args[key];
    if (value === undefined) return undefined;
   ...
   * 文字列配列を取得（オプション）
   */
  optionalStringArray(args: DSLArgs, key: string): string[] | undefined {
    const value = args[key];
    if (value === undefined) return undefined;
   ...
   * 数値配列を取得（オプション）
   */
  optionalNumberArray(args: DSLArgs, key: string): number[] | undefined {
    const value = args[key];
    if (value === undefined) return undefined;
