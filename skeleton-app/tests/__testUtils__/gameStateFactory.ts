 ...
 *
 * ## シンクロ召喚テスト用
 * - SYNCHRO_TEST_CARD_IDS: シンクロ関連テストカードID定数
 ...
 */
export function createSynchroSummonNoTunerState(): GameSnapshot {
 ...
 */
export function createSynchroSummonLevelMismatchState(): GameSnapshot {
  return createMockGameState({
    space: {
      mainDeck: createCardInstances(Array(30).fill(TEST_CARD_IDS.DUMMY), "mainDeck"),
