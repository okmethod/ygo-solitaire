...
describe("GenericIgnitionEffect - インスタンス生成", () => {
  it("createGenericIgnitionEffect でインスタンスを生成できる", () => {
    const effect = createGenericIgnitionEffect(EFFECT_MONSTER_ID, 1, baseDsl());
...
describe("GenericIgnitionEffect - 条件チェック", () => {
  it("メインフェイズ1では canActivate が true を返す", () => {
    ...
    const result = effect.canActivate(stateByPhase(), monsterOnField());
  ...
  it("メインフェイズ以外では canActivate が false を返す", () => {
    ...
    const result = effect.canActivate(stateByPhase("standby"), monsterOnField());
  ...
  it("個別条件を満たさない場合は canActivate が false を返す", () => {
    ...
    const result = effect.canActivate(stateByPhase(), monsterOnField()); // デッキ不足
...
describe("GenericIgnitionEffect - ステップ生成", () => {
  it("createActivationSteps に発動通知ステップが含まれる", () => {
    ...
    const steps = effect.createActivationSteps(stateByPhase(), monsterOnField());
  ...
  it("createResolutionSteps で効果解決ステップを生成できる", () => {
    ...
    const steps = effect.createResolutionSteps(stateByPhase(), monsterOnField());
