...
describe("魔法カード発動 - 基本フローテスト", () => {
  ...
  describe("通常魔法 - 手札から発動（成金ゴブリン）", () => {
    it("発動 → 処理解決 → 墓地へ（effectQueueStore通し）", async () => {
      ...
      const spellId = before.space.hand.find((c) => c.id === ACTUAL_CARD_IDS.GOLDEN_GOBLIN)!.instanceId;
  ...
  describe("フィールド魔法 - 手札から発動（チキンレース）", () => {
    it("発動 → フィールドゾーンに表側表示で配置", async () => {
      ...
      const before = getState();
      const fieldSpellId = before.space.hand.find((c) => c.id === ACTUAL_CARD_IDS.CHICKEN_GAME)!.instanceId;
