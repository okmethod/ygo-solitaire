...
遊戯王カードの 1 ターンキルコンボシミュレータ（固定デッキで先行 1 ターン目をプレイ）
...
- **実装中**: タスク ID（T0xx）をコメントに記載して作業効率を上げる
- **完了後**: ストック情報（ソースコード、`docs/`）からタスク ID を削除
...
- `/speckit.plan` - 実装計画作成
- `/speckit.tasks` - タスク生成
- `/speckit.implement` - **タスク実行 + 自動 tasks.md 更新**
