...
4層Clean Architectureにおける外部リソースへのアクセス層の実装状況。
...
| Port (抽象インターフェース) | Adapter (具象実装) | 実装状況 | 備考 |
|--------------------------|------------------|---------|------|
| `ICardDataRepository` | `YGOProDeckCardRepository` | 🚧 設計完了・実装予定 | Application LayerがInfrastructure Layerに直接依存しないための抽象化。Phase 1設計書参照。 |
