...
**実装箇所**: `domain/models/GameState/Player`
...
**実装箇所**: `domain/models/Card`, `domain/cards`
...
- **Activated CardIds** : そのターンに発動したカード
- **Game Rusult / Victory Decision** : 勝敗判定

**実装箇所**: `domain/models/GameState`, `domain/models/GameSnapshot`
...
**実装箇所**: `domain/models/GameState/Phase`

---

## CardSpace / Location: カード配置空間 / 位置

カードが配置される空間。ゲーム中のすべてのカードは、この空間の中のいずれかの位置に存在する。  
自分/相手のそれぞれに固有の空間があり、空間内には以下の位置がある。

**実装箇所**: `domain/models/GameState/CardSpace.ts`, `domain/models/Location`

### Main Deck: デッキ / メインデッキ / 山札

- 当該ゲームで使われるカードプールの初期配置領域。
- 枚数は公開情報、順番や内訳は非公開情報。

### Extra Deck: エクストラデッキ

- ルール上、メインデッキと区別して扱われるカードプールの配置領域。
...
- **敗北条件**
  - 自分のライフポイントが 0 になる
  - 自分のデッキが 0 枚の状態で、自分がカードをドローする（⏳ 未実装）
  - サレンダー（降参）する（⏳ 未実装）
...
**実装箇所**: `domain/model/GameProcessing`, `application/GameFacade.ts`
...
詳細は [ゲーム操作コマンドモデル](./game-command-model.md) を参照。
...
大別して「発動する効果」と「適用する効果」がある。  
カードタイプや、カード固有のルール・手続きに従い処理する。
