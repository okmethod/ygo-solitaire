...
本アプリケーションで扱う遊戯王 OCG（オフィシャルカードゲーム）の基本ルールを定義します。
...
- デッキの一番上からカードを 1 枚ドローする
- 先攻 1 ターン目は初期手札配布済みのため、ドローしない
...
- 手札から発動
- 発動後は墓地へ送られる
- **Normal Spell (通常魔法)**: メインフェイズ 1 に発動可能

### Trap (罠カード)

- セットして次のターン以降に発動可能
- **本アプリでのスコープ**: 発動判定ルールのみ実装（先攻 1 ターン目は発動不可、手札からの直接発動も不可）
...
- [遊戯王 OCG 公式ルール](https://www.yugioh-card.com/japan/howto/)
  - 本ドキュメントは先攻 1 ターンキルシミュレーターに必要な範囲のみを記載
