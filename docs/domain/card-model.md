...
モンスターカードの使用方法（各種召喚等）についてのルールの詳細は、 [ゲーム操作コマンドモデル](./game-command-model.md) を参照。
...
- 効果のないモンスター＝通常モンスターではない。
  - 例：「効果のないシンクロモンスター」は、「通常モンスター」ではない。
- モンスタートークンはカードの効果で生み出されたモンスターであり、カードではない。
  - カードではないので手札・デッキに戻ることはなく、破壊されたら墓地へは行かずにデュエルから取り除かれる。除外ゾーンにも行かない。
  - トークンは裏側表示になることはない。
  - トークンは常に「通常モンスター」として扱われる。効果を持っているように見えても、それはトークンの効果ではなく、トークンを生み出したカードの効果として扱われる。
  - 特筆された制約がない限り、トークンをリリース・シンクロ素材・リンク素材として使用することができる。しかしトークンをエクシーズ素材とすることはできない。

### Spell: 魔法カード

手札から発動し、さまざまな効果を発揮するカード。フィールド（魔法・罠ゾーン）に配置される。
魔法カードの使用方法（発動・セット等）についてのルールの詳細は、[ゲーム操作コマンドモデル](./game-command-model.md) を参照。
...
罠カードの使用方法（発動・セット等）についての詳細は、[ゲーム操作コマンドモデル](./game-command-model.md) を参照。
...
カードはゲーム中、常にカード配置空間（CardSpace）内のいずれかの位置（Location）に存在する。  
詳細は [overview.md の CardSpace / Location セクション](./overview.md) を参照。
...
特定のカードの効果によって、カードにカウンターが置かれることがある。
カウンターには様々な種類があり、それぞれのカウンターに関連するカードの効果によって様々な効果が適用される。  
（例: 魔力カウンター、黒羽カウンター、武士道カウンター）
