...
- **チェーン管理**:
  ...
  - チェーンできるイベントとタイミングを検知し、チェーン発動可能なカード情報を収集し、チェーン有無を確認するステップをキューに挿入する
  - チェーンする場合は、発動処理ステップを処理し、解決処理ステップをスタックする
  - チェーン解決時に、スタックした解決処理ステップを LIFO 順で処理する
- **UI 連携**:
...
[ステップ実行ループ:開始]
  ...
  [[チェーン確認]]
  チェーンスタックが空でない？
    ├─ Yes → チェーン可能な効果を収集 → チェーン確認ステップを挿入
    │      発動する？
    │        ├─ Yes → チェーン構築を繰り返す
    │        └→ No  → チェーン構築終了 → スタックをLIFO順で解決
    └─ No  → 次のステップへ
[ステップ実行ループ:終了]
    ↓
...
チェーンスタックの SSOT を管理する。  
チェーン構築・解決の状態管理とスペルスピード判定を担当する。

**ファイル**: `application/stores/chainStackStore.ts`

**主要機能**

- **チェーン構築状態管理**: チェーン構築中かどうかを追跡する
- **LIFO スタック操作**: チェーンブロックのスタックのPush/Popを行う
- **スペルスピード判定**: チェーンに必要なスペルスピードを判定する
...
- `routes/(auth)/simulator/[deckId]/_components/`
  ...
  - `modals/ChainConfirmationModal.svelte`: チェーン確認インタラクション
