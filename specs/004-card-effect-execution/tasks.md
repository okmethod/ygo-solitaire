...
- [x] T013 [P] [US1] Unit Test作成: `skeleton-app/tests/unit/CardEffects.test.ts` を新規作成し、Pot of Greed（cardId 55144522）の効果処理テストを追加
...
- [x] T014 [P] [US1] Unit Test追加: `CardEffects.test.ts` にPot of Greedのデッキ枚数チェック（>= 2）テストを追加
  - デッキ1枚の状態で `canExecute()` が false を返すことを検証
- [x] T015 [US1] Unit Test実行: `npm test` で全テスト通過確認（254 tests passed）

**Checkpoint**: User Story 1完了 - 強欲な壺のロジックがUnit Testで検証され（5 tests）、既存E2Eテスト（card-activation.spec.ts）で統合動作確認済み
